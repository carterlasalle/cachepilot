#!/usr/bin/env bash
# E2E-001 Run 13 -- FULL live CLI/API user journey + re-verification of ALL prior
...
LOG=/tmp/r13_journey.log
: > "$LOG"
echo "[r13] journey start $(date -u +%FT%TZ)" | tee -a "$LOG"

# --- guard: fail fast if any stale 908x listener ---
e2e_guard_pre_run --clean || { echo "PRE-RUN GUARD FAILED"; exit 1; }
echo "[r13] pre-run hygiene guard passed (908x clean)" | tee -a "$LOG"
...
SEED_SHA_BEFORE=$(sha256sum /tmp/r13-telemetry.db | cut -d' ' -f1)
echo "[r13] seeded telemetry DB sha24=$SEED_SHA_BEFORE" | tee -a "$LOG"

# --- spawn mock upstream + relay pass-through pair ---
e2e_spawn python e2e-output/run13/mock_upstream.py 9081
sleep 0.8
...
e2e_spawn python e2e-output/run13/upstream_503.py 9092
sleep 0.6
...
e2e_spawn python dashboard/backend/server.py --db /tmp/r13-telemetry.db --port 9083
sleep 0.8
head -c 300 /dev/urandom > /tmp/r13-corrupt.db
e2e_spawn python dashboard/backend/server.py --db /tmp/r13-corrupt.db --port 9086
sleep 0.8
./.venv/bin/python e2e-output/run13/make_wrongschema.py >>"$LOG" 2>&1
e2e_spawn python dashboard/backend/server.py --db /tmp/r13-wrong.db --port 9087
sleep 0.8
# nonexistent --db + nonexistent parent dir (E2E-004: never creates)
rm -rf /tmp/r13-nodir
e2e_spawn python dashboard/backend/server.py --db /tmp/r13-nodir/missing.db --port 9088
sleep 0.8
...
curl -s -D /tmp/r13_gh.txt -o /tmp/r13_gbody.txt "$R/upstream/resource"
grep -i 'X-Upstream-Marker' /tmp/r13_gh.txt >>"$LOG" 2>&1 && echo "  marker present" || echo "  NO MARKER"
echo "  relayGET_body=$(cat /tmp/r13_gbody.txt)" | tee -a "$LOG"

echo "-- C. relay POST pass-through byte-identical (echo body) --" | tee -a "$LOG"
curl -s -D /tmp/r13_ph.txt -o /tmp/r13_pbody.txt -X POST -d '{"payload":"echo-me-13"}' "$R/upstream/posts"
grep -i 'X-Upstream-Marker' /tmp/r13_ph.txt >>"$LOG" 2>&1 && echo "   marker present" || true
echo "  relayPOST_body=$(cat /tmp/r13_pbody.txt)" | tee -a "$LOG"
...
for ep in health status leases costs ttl churn routes topology miss; do
  code=$(curl -s -o /tmp/r13_api_json -w "%{http_code}" "$B/api/$ep")
  size=$(wc -c < /tmp/r13_api_json)
  echo "  /api/$ep -> HTTP $code ${size}B $(head -c 40 /tmp/r13_api_json | tr -d '\n')" | tee -a "$LOG"
done
...
for cmd in status leases costs ttl churn explain-miss routes topology; do
  out=$(uv run --no-sync cachepilot "$cmd" --db /tmp/r13-telemetry.db 2>&1 | head -1)
  echo "  cachepilot $cmd -> $out" | tee -a "$LOG"
done
SEED_SHA_AFTER=$(sha256sum /tmp/r13-telemetry.db | cut -d' ' -f1)
...
CACHEPILOT_RELAY_LISTEN=127.0.0.1:9082 uv run --no-sync cachepilot status --db /tmp/r13-telemetry.db 2>&1 | grep -iE "relay|healthy|ok" | head -1 >>"$LOG"
echo -n "    unreachable: " | tee -a "$LOG"
CACHEPILOT_RELAY_LISTEN=127.0.0.1:9998 uv run --no-sync cachepilot status --db /tmp/r13-telemetry.db 2>&1 | grep -iE "relay|unreach|down|fail" | head -1 >>"$LOG"
...
OCPQ=$!
sleep 0.5
CACHEPILOT_RELAY_LISTEN=127.0.0.1:9091 uv run --no-sync cachepilot status --db /tmp/r13-telemetry.db 2>&1 | grep -iE "relay|occup|" | head -1 | tee -a "$LOG"
...
OCA=$!; sleep 0.5
cachepilotd --listen 127.0.0.1:9089 --upstream http://127.0.0.1:9081 >/tmp/r13_relay_occ.out 2>&1; RC=$?
echo "  cachepilotd occupied exit=$RC ($(head -1 /tmp/r13_relay_occ.out | tr -d '\n'))" | tee -a "$LOG"
...
OCB=$!; sleep 0.5
python dashboard/backend/server.py --db /tmp/r13-telemetry.db --port 9084 >/tmp/r13_dash_occ.out 2>&1; RC2=$?
echo "  server.py occupied exit=$RC2 ($(grep -oE 'already in use' /tmp/r13_dash_occ.out | head -1))" | tee -a "$LOG"
kill $OCB 2>/dev/null; wait $OCB 2>/dev/null

echo "-- E2E-003/007 uniform JSON 405 for POST/PUT/DELETE/PATCH/OPTIONS/TRACE --" | tee -a "$LOG"
for m in POST PUT DELETE PATCH OPTIONS TRACE; do
  hdr=$(curl -s -D - -o /tmp/r13_405.json -X "$m" "$R/cachepilot/health" 2>/dev/null)
  code=$(echo "$hdr" | head -1 | awk '{print $2}')
  ctype=$(echo "$hdr" | grep -i '^content-type' | tr -d '\r' | sed 's/^[Cc]ontent-[Tt]ype: //')
  echo "  $m /cachepilot/health -> HTTP ${code:-000} ctype=${ctype:-?} body=$(cat /tmp/r13_405.json 2>/dev/null | tr -d '\n')" | tee -a "$LOG"
done
...
rm -rf /tmp/r13-noclidb
uv run --no-sync cachepilot status --db /tmp/r13-noclidb/nope.db >/tmp/r13_nodb.out 2>&1; RC=$?
echo "  exit=$RC file_created=$([ -e /tmp/r13-noclidb/nope.db ] && echo YES || echo NO) dir_created=$([ -d /tmp/r13-noclidb ] && echo YES || echo NO)" | tee -a "$LOG"
echo "  output=$(head -1 /tmp/r13_nodb.out | tr -d '\n')" | tee -a "$LOG"

echo "-- E2E-005 churn-vs-switches disambiguated --" | tee -a "$LOG"
echo -n "  status churn/footnote: " | tee -a "$LOG"
uv run --no-sync cachepilot status --db /tmp/r13-telemetry.db 2>&1 | grep -iE "churn" | head -2 | tr '\n' ' | ' >>"$LOG"; echo >>"$LOG"
echo -n "  routes switches: " | tee -a "$LOG"
uv run --no-sync cachepilot routes --db /tmp/r13-telemetry.db 2>&1 | grep -iE "switch" | head -1 | tr -d '\n' | tee -a "$LOG"; echo >>"$LOG"

echo "-- E2E-008/009 corrupt + wrong-schema: 8 CLI read commands exit 0 no traceback --" | tee -a "$LOG"
for bad in /tmp/r13-corrupt.db /tmp/r13-wrong.db; do
  for cmd in status leases costs ttl churn explain-miss routes topology; do
...
for port in 9086 9087; do
  code=$(curl -s -o /tmp/r13_bad_api.json -w "%{http_code}" "http://127.0.0.1:$port/api/leases")
  echo "  :$port/api/leases -> HTTP $code body=$(cat /tmp/r13_bad_api.json | tr -d '\n')" | tee -a "$LOG"
done
