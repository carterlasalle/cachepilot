#!/usr/bin/env bash
# Focused E2E-002 relay-readout evidence capture (Run 14).
set -u
cd /home/hermes/cachepilot || exit 1
export PATH="$PWD/.venv/bin:$PATH"
source e2e-output/hygiene.sh
LOG=/tmp/r14_relayreadout.log; : > "$LOG"
...
e2e_spawn python e2e-output/run14/mock_upstream.py 9081; sleep 0.7
e2e_spawn cachepilotd --listen 127.0.0.1:9082 --upstream http://127.0.0.1:9081; sleep 0.8
echo "=== healthy relay (9082) ===" | tee -a "$LOG"
CACHEPILOT_RELAY_LISTEN=127.0.0.1:9082 uv run --no-sync cachepilot status --db /tmp/r14-telemetry.db 2>&1 | grep -iE "relay *: *|relay |reachable" | tee -a "$LOG"
echo "=== unreachable (9998 closed) ===" | tee -a "$LOG"
CACHEPILOT_RELAY_LISTEN=127.0.0.1:9998 uv run --no-sync cachepilot status --db /tmp/r14-telemetry.db 2>&1 | grep -iE "relay *: *|unreach|down|fail" | tee -a "$LOG"
...
OCP=$!
sleep 0.5
CACHEPILOT_RELAY_LISTEN=127.0.0.1:9091 uv run --no-sync cachepilot status --db /tmp/r14-telemetry.db 2>&1 | grep -iE "relay *: *|occup|another|in use|not " | tee -a "$LOG"
