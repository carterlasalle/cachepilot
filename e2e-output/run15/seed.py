"""E2E-001 Run 15 seed helper — writes a seeded telemetry DB for CLI/API checks.
...
def main() -> None:
    db = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("e2e-output/run15/telemetry.db")
    seed_store(db)
